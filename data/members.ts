import type { BandMember } from '@/types/member';

export const bandMembers: BandMember[] = [
  {
    slug: 'felipe',
    name: 'Felipe Gardenghi',
    role: 'Vocal / Guitarra',
    image: '/images/integrantes/Felipe.jpg',
    imagePosition: 'scale-[1.16] origin-top',
    bioParagraphs: [
      'Músico desde os 13 anos, já participou de diversas bandas locais e projetos paralelos, sempre explorando o universo do rock, do pop e do emo. Já abriu shows de bandas relevantes do cenário emo, como Fresno e McFly.',
      'Dentre algumas bandas em que passou, estão Patrulha 7 (lançando um álbum autoral, como baixista), Flop, Emopala, Hellboys (alguns singles lançados, também como baixista), Blue Drive, Rafael Puga Trio, entre várias outras — além de ter conhecido artistas renomados no cenário nacional, como NX Zero, Supercombo, Gustavo Bertoni (Scalene) e Emmily Barreto (Far From Alaska e Ego Kill Talent), e ter dividido palco com bandas e artistas como Linkin Park Cover Ribeirão, Kilotones, Ciça Moreira, Soberba, Green Day Cover Ribeirão, e mais.',
      'Além da banda, é desenvolvedor de software, cientista de dados e professor de música, e ainda teve uma breve carreira como Streamer de jogos indie na Twitch, onde ocasionalmente ainda aparece para interagir com a comunidade e compartilhar momentos divertidos e significativos entre antigos e novos amigos. Em 2023, fundou a banda Revanche para dar voz a um tributo Fresno e espalhar a palavra do emo por onde passar.',
    ],
    socials: {
      instagram: 'https://instagram.com/felipegardenghi',
      twitter: 'https://twitter.com/felipegardenghi',
      tiktok: 'https://tiktok.com/@felipe.gardenghi',
      twitch: 'https://twitch.tv/felipegardenghi',
    },
  },
  {
    slug: 'eduardo',
    name: 'Eduardo Opaleiro',
    role: 'Guitarra / Voz de Apoio',
    image: '/images/integrantes/Eduardo.jpg',
    bioParagraphs: [
      'Músico desde os 17 anos, começou no violão e se apaixonou de vez pela guitarra no primeiro impacto com o Nirvana. Já rodou o Brasil integrando diversas bandas, sempre em busca de identidade, peso e sentimento. Fã declarado de pedaleiras digitais e da clássica Stratocaster, transita com naturalidade entre blues, hip hop, nu metal, punk rock, grunge e outras vertentes que moldaram sua sonoridade.',
      'Além da Revanche, também é guitarrista da Soberba e da Green Day Cover Ribeirão. E dentre algumas outras bandas em que passou, estão The Donuts, Emopala, For Psychos, entre outras. Abriu o show de bandas importantes da cena, como Fresno, McFly, Marcelo Mancini (Strike), Esteban Tavares, além de ter conhecido vários artistas de alcance nacional, e ter dividido palco com Ciça Moreira.',
      'Fora da banda, é instrutor de golfe, e uma enciclopédia ambulante de curiosidades e histórias sobre os mais variados assuntos possíveis. E falando em curiosidade, você sabia que "Opaleiro" não é o sobrenome dele? Na verdade, é um apelido que surgiu por causa do seu amor por carros Opala, e acabou pegando tanto que virou parte da sua identidade artística.',
    ],
    socials: {
      instagram: 'https://instagram.com/luidskt',
    },
  },
  {
    slug: 'yago',
    name: 'Yago Borges',
    role: 'Baixo / Voz de Apoio',
    image: '/images/integrantes/Yago.jpg',
    bioParagraphs: [
      'Músico desde os 16 anos, tem uma trajetória marcada pela paixão pela música e pela integridade aos seus princípios. Já abriu shows de artistas relevantes da cena, como o Bruno Sutter em sua turnê de homenagem ao Iron Maiden.',
      'Além da Revanche, também é baixista da Nevermind (Nirvana Cover) ganhando premiações importantes na cena rock do interior paulista.',
      'Fora da banda, esbanja carisma e piadas incríveis, além de amar qualquer bebida azul que encontrar.',
    ],
    socials: {
      instagram: 'https://instagram.com/kdodk138',
    },
  },
  {
    slug: 'leonan',
    name: 'Leonan Artal',
    role: 'Bateria',
    image: '/images/integrantes/Leonan.jpg',
    bioParagraphs: [
      'Músico desde os 12 anos, consolidou sua trajetória como baterista na Revanche, trazendo potência, precisão e presença de palco para a banda.',
      'Além da Revanche, é vocalista/guitarrista da Green Day Cover Ribeirão, e multi-instrumentista/compositor na Crems, expandindo sua atuação dentro do universo do rock e suas vertentes.',
      'Fora da banda, é empresário e gestor de tráfego, unindo criatividade e performance com estratégia e visão de negócios.',
    ],
    socials: {
      instagram: 'https://instagram.com/leo.artal',
    },
  },
];
